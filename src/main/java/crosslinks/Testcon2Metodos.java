...
public class Testcon2Metodos {
	...
	{
		Testcon2Metodos wikipedia = new Testcon2Metodos();
		wikipedia.setUp();
	...
	private List<String> FindoneNode() 
	{
		...
		try (Transaction tx = graphDb.beginTx()) {
			Node start = graphDb.findNode(recordClassLabel, "wikiid", "765");
			FindCl(graphDb, start, NumberCross, ListNode);
			tx.success(); 
	...
	public static List<String> FindCl(GraphDatabaseService graphDb, Node start ,int NumberCross, List<String> ListNode) { 
